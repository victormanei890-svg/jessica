...
const Hero: React.FC<HeroProps> = ({ data }) => {
  return (
    <section id="home" className="relative min-h-screen flex items-center pt-24 pb-12 overflow-hidden bg-gradient-to-br from-[#FDFCFB] to-[#F3F5F7]">
      {/* Decorative Elements */}
      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-10 md:gap-12 items-center relative z-10">
        ...
        <div className="order-1 md:order-2 flex justify-center animate-in fade-in zoom-in duration-1000">
          <div className="relative group w-full max-w-[280px] sm:max-w-[320px] md:max-w-sm">
            ...
            <div className="relative w-full aspect-[4/5] overflow-hidden rounded-3xl shadow-2xl bg-white flex items-center justify-center">
              {data.heroImage ? (
                <img 
                  src={data.heroImage} 
                  alt={data.name}
                ...
                <div className="flex flex-col items-center justify-center text-rose-200 space-y-4">
                  <User size={100} strokeWidth={1} />
                  <span className="text-xs font-medium uppercase tracking-widest text-slate-300">Aguardando foto</span>
                </div>
              )}
            </div>
          </div>
