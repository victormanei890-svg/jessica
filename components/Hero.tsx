...
const Hero: React.FC<HeroProps> = ({ data }) => {
  // Garantia absoluta de que o WhatsApp terá apenas números para o link
  const cleanWhatsapp = data.contact.whatsapp.replace(/\D/g, '');

  return (
    <section id="home" className="relative min-h-screen flex items-center pt-24 pb-12 overflow-hidden bg-gradient-to-br from-[#FDFCFB] to-[#FDF5F5]">
      {/* Decorative Elements */}
      <div className="absolute top-20 right-[-10%] w-[400px] h-[400px] bg-rose-100 rounded-full blur-3xl opacity-30" />
      <div className="absolute bottom-20 left-[-5%] w-[300px] h-[300px] bg-rose-50 rounded-full blur-3xl opacity-30" />
      ...
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-10 md:gap-12 items-center relative z-10">
        <div className="order-2 md:order-1 animate-in fade-in slide-in-from-left-8 duration-700">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <a 
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-rose-500 text-white px-8 py-4 rounded-full font-semibold flex items-center justify-center gap-2 hover:bg-rose-600 transition-all shadow-xl shadow-rose-200"
            >
        ...
        <div className="order-1 md:order-2 flex justify-center animate-in fade-in zoom-in duration-1000">
          <div className="relative group w-full max-w-[280px] sm:max-w-[320px] md:max-w-sm">
            <div className="absolute -inset-4 bg-rose-100/30 rounded-3xl blur-2xl transition-colors duration-500" />
            <div className="relative w-full aspect-[4/5] overflow-hidden rounded-3xl shadow-2xl bg-white/50 flex items-center justify-center">
              {data.heroImage ? (
                <img 
                  src={data.heroImage} 
                  alt={data.name}
