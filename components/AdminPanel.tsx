    ...
    <div className="fixed inset-0 z-50 bg-slate-100 flex overflow-hidden flex-col lg:flex-row">
      ...
      <div className="lg:w-80 bg-white border-r border-slate-200 p-8 hidden lg:flex flex-col">
        ...
        <div className="mt-auto pt-6 border-t border-slate-100">
           <button onClick={onClose} className="w-full py-3 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 transition-colors">Sair sem Salvar</button>
        </div>
      </div>
      ...
      <div className="flex-grow flex flex-col h-full bg-slate-50 overflow-hidden">
        <header className="bg-white border-b border-slate-200 px-6 md:px-8 py-4 flex justify-between items-center shrink-0">
          <h1 className="font-bold text-slate-800 truncate">Configurações</h1>
          <div className="flex gap-2">
            ...
            <button onClick={handleSave} disabled={savedSuccess} className={`flex items-center gap-2 px-4 md:px-6 py-2 rounded-full font-bold transition-all shadow-md ${savedSuccess ? 'bg-green-500 text-white' : 'bg-rose-500 text-white hover:bg-rose-600'}`}>
                {savedSuccess ? <><Check size={18} /> Salvo!</> : <><Save size={18} /> Salvar</>}
            </button>
          </div>
        </header>
        ...
        <main className="flex-grow overflow-y-auto p-4 md:p-12">
          <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300 space-y-8">
                ...
                <div className="space-y-8">
                    <section className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-sm font-bold text-slate-500 mb-2">Nome Exibido</label>
                    ...
                    <section className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-center bg-slate-50 p-6 rounded-2xl border border-dashed border-slate-300">
                            <div className="w-32 h-40 rounded-xl overflow-hidden shrink-0 bg-white shadow-md border border-slate-200 flex items-center justify-center">
                                {editData.heroImage ? <img src={editData.heroImage} className="w-full h-full object-cover" alt="Preview" /> : <User className="text-slate-200" size={40} />}
                            </div>
                            <div className="flex-grow w-full space-y-4">
                                ...
                                <div className="flex gap-2">
                                  ...
                                  {editData.heroImage && <button onClick={() => setEditData({...editData, heroImage: ""})} className="px-4 py-2 text-sm font-bold text-red-500 hover:underline">Remover</button>}
                                </div>
                                <p className="text-xs text-slate-400">Tamanho recomendado: 800x1000px.</p>
                            </div>
                        </div>
                    </section>
                    <section className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
                        ...
                        <textarea rows={3} value={editData.quote} onChange={(e) => setEditData({...editData, quote: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-500 italic" />
                    </section>
                    <section className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
                        <h3 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-4">Biografia</h3>
            ...
            {activeTab === 'hero' && (
                <section className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
                    <h3 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-4">Textos da Seção Inicial</h3>
                    <div className="space-y-6">
                        <div>
                          <label className="block text-sm font-bold text-slate-500 mb-2">Badge Superior (Texto pequeno colorido)</label>
                        ...
                        <div>
                          <label className="block text-sm font-bold text-slate-500 mb-2">Título Principal (Pode usar enter para quebra de linha)</label>
            ...
            {activeTab === 'approach' && (
                <section className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
                    <h3 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-4">Abordagem</h3>
                    <div className="space-y-6">
                        <div>
                          <label className="block text-sm font-bold text-slate-500 mb-2">Título</label>
                        ...
                        <div>
                          <label className="block text-sm font-bold text-slate-500 mb-2">Descrição</label>
                          <textarea rows={8} value={editData.approachDescription} onChange={(e) => setEditData({...editData, approachDescription: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl" />
                        </div>
                    </div>
                </section>
            )}
            ...
            {activeTab === 'services' && (
                <section className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
                    <h3 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-4">Serviços / Tags</h3>
                ...
                <section className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200 shadow-sm space-y-6">
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex gap-3 mb-6">
                        ...
                        <p className="text-xs md:text-sm text-blue-700 leading-relaxed">
                            <strong>Importante:</strong> No campo WhatsApp, insira o número com <strong>55 + DDD + Número</strong> sem espaços ou traços para o link funcionar corretamente.
                        </p>
                    </div>
                    <div className="grid md:grid-cols-2 gap-6">
                        <div className="md:col-span-2">
                            <label className="block text-sm font-bold text-slate-500 mb-1">WhatsApp (Apenas Números: 55...) </label>
                            <input type="text" value={editData.contact.whatsapp} onChange={(e) => setEditData({...editData, contact: {...editData.contact, whatsapp: e.target.value.replace(/\D/g,'')}})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-mono" />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-slate-500 mb-2">Telefone Visível</label>
                            <input type="text" value={editData.contact.phone} onChange={(e) => setEditData({...editData, contact: {...editData.contact, phone: e.target.value}})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl" />
                        </div>
                        <div>
                        ...
                        <div className="md:col-span-2">
                          <label className="block text-sm font-bold text-slate-500 mb-2">Endereço</label>
